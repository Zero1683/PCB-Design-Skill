...
- Before modifying an external whole-board baseline, record a `source-baseline.json` in the project and run `python scripts/source_inventory.py --root /project`. The manifest records `schema: 1`, exact HTTPS `source_url`, pinned `revision`, timezone-aware `retrieved_at`, `kind` (`editable_board` or `reference_only`), and a `files` list. Each file has a project-relative `path`, `role` (`schematic`, `pcb`, `license_evidence`, `footprint`, `bom`, `manufacturing`, `documentation`, or `other`) and SHA-256. An editable board requires schematic, PCB and local license evidence; a reference-only source still needs license evidence. Keep the source snapshot immutable and use a separate working copy. The helper rejects missing, changed, linked and escaping paths; its positive result says nothing about whether the license covers the planned output or whether the design works.
