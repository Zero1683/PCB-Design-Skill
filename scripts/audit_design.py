#!/usr/bin/env python3
"""Compare normalized component/pin records and screen board-coordinate body envelopes."""
import argparse
import json
import math
...
def load_snapshot(data):
    if data.get('schema') != 1 or data.get('kind') not in {'schematic', 'pcb', 'bom'}:
    ...
    for part in parts:
        ...
        if data['kind'] != 'bom':
            ...
            if any(not k or (v is not None and (not isinstance(v, str) or not v)) for k, v in pins.items()):
...
def main():
    ...
    try:
        result=compare(read(args.left),read(args.right)) if args.mode=='compare' else geometry(read(args.pcb),args.clearance_mm)
    except (OSError,ValueError,KeyError,TypeError) as exc:p.exit(2,f'ERROR: {exc}\n')
    print(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False))
    return int(not result['records_match']) if args.mode=='compare' else int(bool(result['suspect_pairs'] or result['missing_geometry']))
