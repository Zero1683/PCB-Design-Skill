...
**告诉 AI 你想做什么，然后亲手把它做出来。**

你可以从一句“我想做一个编程用的小键盘”开始。Agent 帮你梳理功能、尺寸和供电，查找适用的成熟电路，列出元件预算，再完成原理图、PCB 和制造文件。每个阶段说明已完成的工作、接下来做什么，以及需要你配合的事情。专业计算和检查留在背后，关键取舍用日常语言解释。

交付时，你会知道哪个文件用于打板、需要哪些元件、如何装配和开始调试。已有电路会注明来源、适用条件和修改内容；未完成的固件、外壳或实物测试会明确列出。项目开源，希望让没有硬件基础的人也能开始动手，并把经验留给后来者。

内置 EasyEDA API、原理图辅助方法库、本地桥接服务及运行依赖，支持嘉立创EDA中的工程操作。详细工程记录和检查报告保存在项目中，方便复查和继续制作。
...
这一版也收录了开发版的 PCBA 专项复核：贴装清单和坐标、工作状态、生成板型、衍生板变更，以及制造文件的版本与哈希。完整结果留在工程目录，日常更新只给出短摘要。详见[检查方法与数据范围](references/36-project-reviews.md)及[验证记录](VALIDATION.md)。

离线检查可批量运行并复用未变化的输入；尺寸检查将用户接受的尺寸与实际板框对应。实时 EDA 状态、DRC 和商城报价不会当作缓存的当前结果。见[本地执行](references/33-local-execution.md)及[尺寸核对](references/34-mechanical-envelope.md)。

在一个 150 器件的合成检查案例中，完整报告为 19,572 字节，返回摘要为 604 字节，重复执行命中缓存。所有错误仍保留在报告中。这是输出量与缓存测试，尚未测得完整画板任务的额度节省比例。

v1.8.0 发布时，409 项 Python 测试、81 项 Node 测试及独立桥接冒烟检查通过，无跳过项；另完成 `test1` 原生移动、拦截、保存重开和回滚验证。
...
下载 [v2.0.0 完整安装包](https://github.com/Zero1683/PCB-Design-Skill/releases/download/v2.0.0/PCB-Design-Skill-v2.0.0.zip)。也可克隆仓库：
...
第一次使用可以只说“我想做一个能用的小键盘，之前没画过 PCB”。Agent 会先问少量与你实际使用有关的问题，再给出有尺寸和元件预算的方案。需要你决定的事项会集中说明；引脚核对、计算、绘图和检查由 Agent 处理。已有工程可以从上次阶段继续，不用重答已确认的需求。
...
继续已有工程时，Agent 可运行 `python scripts/project_progress.py --root <工程目录> --baseline <设计版本> --lang zh` 获取简短的当前阶段和下一步；完整检查仍用原有证据文件。复用外部工程时，`python scripts/source_inventory.py --root <工程目录>` 核对下载快照，授权范围和电路适用性另行审查。
